...
class OpenAiClient(
    ...
    private fun planPreviewPrompt(request: RelayPlanRequest, contextBudgetTokens: Int): String {
        ...
        Artifact goal contract: for Docs, Sheets, and Notion-style surfaces, model the goal as resolve target -> navigate/select/edit -> verify end state -> DONE. Read-only search tools never satisfy navigation, cursor, selection, or editing goals by themselves.
        For document navigation like "go to Meetings" or "move cursor to Meetings", use ARTIFACT_RESOLVE_TARGET, then ARTIFACT_NAVIGATE_TO_TARGET or ARTIFACT_SET_CURSOR_AT_TARGET, then ARTIFACT_VERIFY_END_STATE with requiredEndState="cursor_at_target" or "viewport_at_target" before DONE.
        For document edits, prefer DOC_INSERT_AT_TARGET, DOC_REPLACE_TARGET_TEXT, DOC_DELETE_TARGET_TEXT, DOC_APPLY_FORMAT, DOC_MOVE_BLOCK, DOC_CREATE_SECTION, DOC_GET_TARGET_METADATA, and DOC_EXTRACT_ACTION_ITEMS over raw typing when a target label is known.
        For spreadsheet work, prefer SHEET_RESOLVE_RANGE, SHEET_SET_RANGE_VALUES, SHEET_APPEND_TABLE_ROW, SHEET_UPDATE_ROW_BY_MATCH, SHEET_APPLY_FORMULA, SHEET_SORT_FILTER_RANGE, SHEET_INSERT_DELETE_ROWS_COLUMNS, and SHEET_VALIDATE_TABLE_STATE.
        For Notion-like pages/databases, prefer NOTION_RESOLVE_BLOCK_OR_PAGE, NOTION_CREATE_PAGE_OR_BLOCK, NOTION_UPDATE_DATABASE_ITEM, and NOTION_MOVE_OR_REORDER_BLOCK.
        ...
        Device context may include structuredCollections for list-like apps (documents, files, emails, chats, settings). Treat collection items as primary grounding for choosing between similar visible labels. Prefer item.tapTargetNodeId for OPEN_ROW items; do not use generic TAP_TEXT for structured items when a node-backed item action exists. If the best item is TEXT_ONLY/TEXT_GESTURE or candidates are close, search/scroll or ask instead of guessing.
        ...
        If the goal is to open, navigate, go, jump, scroll, find, or search within the current Google Docs, Sheets, or Drive artifact and artifactContext already contains a matching target label, do not OPEN_APP. Prefer NAVIGATE_TO_ARTIFACT_TARGET with {"label":"target text","nodeId":"optional visible node id","kind":"optional target kind","reason":"why"}.
    ...
    private fun planActionPrompt(request: RelayPlanRequest, contextBudgetTokens: Int): String {
        ...
        Artifact goal contract: for Docs, Sheets, and Notion-style surfaces, model the goal as resolve target -> navigate/select/edit -> verify end state -> DONE. Read-only search tools never satisfy navigation, cursor, selection, or editing goals by themselves.
        For document navigation like "go to Meetings" or "move cursor to Meetings", use ARTIFACT_RESOLVE_TARGET, then ARTIFACT_NAVIGATE_TO_TARGET or ARTIFACT_SET_CURSOR_AT_TARGET, then ARTIFACT_VERIFY_END_STATE with requiredEndState="cursor_at_target" or "viewport_at_target" before DONE.
        For document edits, spreadsheet range updates, and Notion block/database updates, use the DOC_*, SHEET_*, and NOTION_* artifact tools instead of raw typing when the target can be resolved.
        Confidence policy: HIGH may execute directly when low risk; MEDIUM may execute only when reversible and low risk; LOW must gather more observation, search, OCR, or ask the user before mutating; any confidence with high risk must ask for confirmation.
        If blocked by risk, ambiguity, credentials, payment, private sharing, deletion, install, or permission changes, ask for confirmation.
        If Device context includes artifactContext, treat it as the primary source for current document, spreadsheet, or folder navigation. Inspect artifactContext.navigationTargets, artifactContext.contentWindow, artifactContext.surface, and artifactContext.availableTools before deciding to launch another app.
        Device context may include accessibilityContentContext for high-limit extracted accessibility text across any app. Treat accessibilityContentContext.contentWindow.fullText and lines as the primary generalized text source when app-specific windows are partial or missing.
